...
### CPU dispatch to reference implementations

Environment setting `DNNL_MAX_CPU_ISA=VANILLA` is best done with a special
build configured with `cmake -DDNNL_ENABLE_MAX_CPU_ISA_VANILLA`.  This tells
libdnnl to include reference implementations even for some cases that should
be completely handled by JIT-ed code.
