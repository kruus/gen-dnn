      ...
      for (int oh = 0; oh < OH; ++oh) {
        for (int ow = 0; ow < OW; ++ow) {
...
#if 1
          kh_beg = (oh*SH < PH? (PH - oh*SH + (DH - 1)) / DH: 0);
#else
#if 0
         ...
         if( oh*SH + 1 <= PH ) kh_beg = (p->dh + PH - oh * SH) / DH; else kh_beg = 0;
          ...
          RT_ASSERT( (kh_end == KH) == (oh*SH + KH*DH < IH+PH+DH) );
          RT_ASSERT( (kh_end >= 1)   == (oh*SH < IH+PH) );
#elif 0
          if (oh*SH + KH*DH < IH + PH + DH) // kh_end>=KH
            kh_end = KH;
          else if (oh*SH < IH+PH) // kh_end >= 1
            kh_end = (IH+PH+ DH-1 - oh*SH) / DH;
          else
            kh_end = 0;
#elif 1 // SX ALL 9.55x
          kh_end = 0;
