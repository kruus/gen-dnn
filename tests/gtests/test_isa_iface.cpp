...
#define DNNL_CPU_ISA_T CONCAT2(dnnl_cpu_isa_,PUBLIC_ISA) /* MUST use dnnl_isa_FOO dnnl.h value */
#define CXX_CPU_ISA    (dnnl::cpu_isa::PUBLIC_ISA)
    static_assert( (int)CXX_CPU_ISA == (int)DNNL_CPU_ISA_T, "cpu_isa enum mismatch in dnnl.hpp?" );
...
TEST(isa_set_once_test, TestISASetOnce) {
    ...
    cout<<" Test using dnnl_cpu_isa_t "<<STR(DNNL_CPU_ISA_T)
            " = 0x"<<hex<<DNNL_CPU_ISA_T<<dec<<endl;

    impl::cpu::cpu_isa_t cpuisa_flags = impl::cpu::from_dnnl(DNNL_CPU_ISA_T);
    printf("%s 0x%lx --> cpu_isa_t cpuisa_flags = 0x%lx\n", STRINGIFY(DNNL_CPU_ISA_T),
            (long)DNNL_CPU_ISA_T, (long)cpuisa_flags);
