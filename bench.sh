...
(cd ${BENCHDIR} && ./benchdnn --conv --mode=CP -v${VERBOSITY} --cfg=f32 --dir=FWD_D --skip-impl="$skip" \
    mb12_ic3ih227iw227_oc96oh55ow55_kh11kw11_sh4sw4ph0pw0_nalexnet:conv1 \
    ) || { echo "Ohoh"; }
(cd ${BENCHDIR} && ./benchdnn --conv --mode=APCT -v${VERBOSITY} --cfg=f32 --dir=FWD_D --skip-impl="$skip" \
...
if [ `uname` == 'SUPER-UX' -o "${DOQUICK}" = "y" ]; then
	...
	(cd ${BENCHDIR} && ./benchdnn --conv --mode=P -v3 --cfg=f32 --dir=FWD_B --skip-impl="$skip" \
	...
	LGBASE="${LOGDIR}/bench-convC-${DOTARGET}t${THREADS}"
	(cd ${BENCHDIR} && ./benchdnn --conv          -v3 --cfg=f32 --dir=FWD_B --skip-impl="$skip" \
        ) 2>&1 | tee ${LGBASE}-tmp.log \
	&& mv ${LGBASE}-tmp.log ${LGBASE}.log && echo "${LGBASE}.log OK" \
	|| { echo "${LGBASE}-tmp.log ERROR"; exit; }
fi
