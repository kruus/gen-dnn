...
void find_min_max(const std::vector<T> &v, float &min_value, float &max_value) {
    min_value = max_value = (float)v[0];
#if DBG
    for(int i=0; i<v.size(); ++i){
        float const vi = (float)v[i];
        if (vi > max_value) max_value = vi;
        if (vi < min_value) min_value = vi;
        printf("v[%3d] = %g\n", i, vi);
    }
#else
    for (auto &e : v) {
...
void compute_q10n_params(const char *message, const std::vector<float> &v,
    ...
    printf("\tComputing q10n params for %s\n"
           ...
           "\t\tZero point:%d\n\n",
            ...
            min_val, max_val, scale, 1 / scale, zp);
}
...
#if DBG
        if (diff > 0.73) {
            printf(" [%3d]  float ref %-12g dequant %-12g diff %-12g\n",
                   n, v1[n], v2_n, diff);
        }
#endif
    }
...
#if DBG
    printf(" M, K, N = %lu, %lu, %lu\n",(long)M,(long)K,(long)N);
    for(int i=0; i<std::min(long{20},std::min(M*K, std::min(K*N, M*N))); ++i){
        printf(" A,B,C[%2d] = %8lu %8ld %g\n",i,(long unsigned)A_u8[i],(long int)B_s8[i],C_f32[i]);
    } 
#endif
...
void compare_f32_and_quantized_matmuls() {
    ...
    // Compute quantized variant (dynamic)
    {
...
#if DBG
        printf( "C_f32 ref scale %g zp %d thresh", scale_C_dynamic_q10n, zp_C_dynamic_q10n, threshold_dynamic_q10n);
        for(int i=0; i<20; ++i){
            printf(" ref C_F32[%2d] = %g-12g  cf C_u8 %u\n", i, C_f32[i], (unsigned)C_u8_dynamic_q10n[i]);
        }
#endif
        ...
        if (rc){
#if DBG
            printf("Dynamic quantization accuracy failed.\n");
#else
            throw std::logic_error("Dynamic quantization accuracy failed.");
#endif
        }
    }
