...
#define POOL_OSZ ((CNV_OSZ)/2)/*27*/
     ...
     * strides: {4, 4}
     */
    memory::dims conv1_src_tz = {batch, 3, SRC_PIX, SRC_PIX};
    memory::dims conv1_weights_tz = {96, 3, CNV_WID, CNV_WID};
    memory::dims conv1_bias_tz = {96};
    memory::dims conv1_dst_tz = {batch, 96, CNV_OSZ, CNV_OSZ};
    memory::dims conv1_strides = {SRC_STRIDE, SRC_STRIDE};
    auto conv1_padding = {0, 0};

    /* Allocate input and output buffers for user data */
    std::vector<float> user_src(batch * 3 * SRC_PIX * SRC_PIX);
...
#else
    TRACE("create lrn1 primitive and add it to net");
    ...
    /* AlexNet: pool1           {batch, 96, 55, 55} -> {batch, 96, 27, 27}
     */
    memory::dims pool1_dst_tz = { batch, 96, POOL_OSZ, POOL_OSZ };
    ...
    * strides: {1, 1}
    */
    memory::dims conv2_src_tz = { batch, 96, POOL_OSZ, POOL_OSZ };
    memory::dims conv2_weights_tz = { 2, 128, 48, 5, 5 };
    memory::dims conv2_bias_tz = { 256 };
    memory::dims conv2_dst_tz = { batch, 256, POOL_OSZ, POOL_OSZ };
