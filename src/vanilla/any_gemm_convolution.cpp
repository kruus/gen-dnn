...
#include "any_gemm_convolution.hpp"
#include "utils.hpp"
...
void _any_gemm_convolution_fwd_t<with_relu>::execute_forward() {
    auto src = reinterpret_cast<const data_t *>(this->input_memory(0));
    auto weights = reinterpret_cast<const data_t *>(this->input_memory(1));
    auto bias = reinterpret_cast<const data_t *>(this->input_memory(2));
    auto dst = reinterpret_cast<data_t*>(this->memory());

    any_gemm_conv_conf_t &jcp = this->conf_.jcp_;
...
template void _any_gemm_convolution_fwd_t<true>::execute_forward();
template void _any_gemm_convolution_fwd_t<false>::execute_forward();

void any_gemm_convolution_bwd_data_t::execute_backward_data() {
...
void any_gemm_convolution_bwd_weights_t::execute_backward_weights() {
    auto src = reinterpret_cast<const data_t *>(this->input_memory(0));
...
#pragma omp parallel num_threads(num_thr)
    {
        ...
        any_gemm_convolution_utils::bwd_weights_balance(ithr, nthr,
        ...
        if (ithr_g != -1 && ithr_mb != -1) {
            ...
            if (need_reduction) {
                ...
                any_gemm_convolution_utils::bwd_weights_reduction_par(
