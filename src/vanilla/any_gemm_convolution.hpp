...
*******************************************************************************/
/** \brief
 * remove jit part of jit_gemm_convolution */
#ifndef CPU_ANY_GEMM_CONVOLUTION_HPP
#define CPU_ANY_GEMM_CONVOLUTION_HPP

#include "c_types_map.hpp"
#include "cpu_convolution_pd.hpp"
#include "cpu_engine.hpp"
#include "any_gemm_convolution_utils.hpp"
...
struct _any_gemm_convolution_fwd_t: public cpu_primitive_t {
    struct pd_t: public _cpu_convolution_fwd_pd_t<with_relu> {
        ...
        DECLARE_COMMON_PD_T(_any_gemm_convolution_fwd_t<with_relu>);
    ...
    _any_gemm_convolution_fwd_t(const pd_t *pd, const input_vector &inputs,
        ...
        : cpu_primitive_t(&conf_, inputs, outputs), conf_(*pd)
    {
        ...
        any_gemm_convolution_utils::init_conf(conf_.jcp_,
            *(conf_.cdesc()), conf_.src_pd(), conf_.weights_pd(0),
            conf_.dst_pd(), with_relu, conf_.negative_slope());
        any_gemm_convolution_utils::prepare_workspace(this->conf_.jcp_,
            &this->ws, false, 0L);
    }
    ~_any_gemm_convolution_fwd_t() {
        //delete sgemm_;
...
using any_gemm_convolution_fwd_t = _any_gemm_convolution_fwd_t<false>;
using any_gemm_convolution_relu_t = _any_gemm_convolution_fwd_t<true>;

struct any_gemm_convolution_bwd_data_t: public cpu_primitive_t {
    struct pd_t: public cpu_convolution_bwd_data_pd_t {
        ...
        DECLARE_COMMON_PD_T(any_gemm_convolution_bwd_data_t);
    ...
    any_gemm_convolution_bwd_data_t(const pd_t *pd, const input_vector &inputs,
              const output_vector &outputs)
        : cpu_primitive_t(&conf_, inputs, outputs), conf_(*pd)
    {
        using namespace prop_kind;
    ...
    ~any_gemm_convolution_bwd_data_t() {
        //delete sgemm_;
        if (this->ws) free(this->ws);
    };
...
struct any_gemm_convolution_bwd_weights_t: public cpu_primitive_t {
    struct pd_t: public cpu_convolution_bwd_weights_pd_t {
        ...
        DECLARE_COMMON_PD_T(any_gemm_convolution_bwd_weights_t);
    ...
    any_gemm_convolution_bwd_weights_t(const pd_t *pd, const input_vector &inputs,
              const output_vector &outputs)
        : cpu_primitive_t(&conf_, inputs, outputs), conf_(*pd)
    {
        using namespace prop_kind;
    ...
    ~any_gemm_convolution_bwd_weights_t() {
        //delete sgemm_0;
        //delete sgemm_1;
        if (this->ws) free(this->ws);
     };
