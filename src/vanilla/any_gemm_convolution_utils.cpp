...
namespace any_gemm_convolution_utils {
    ...
    // FIXME identical with jit_gemm_convolution_utils
void im2col(
    any_gemm_conv_conf_t &jcp, const float *im, float *col) {
...
#pragma omp parallel for  num_threads(num_thr)
    ...
    // FIXME identical with jit_gemm_convolution_utils
void col2im(
    any_gemm_conv_conf_t &jcp, const float *col, float *im) {
    const size_t col_step = jcp.ks * jcp.os;
    const size_t im_step = jcp.ih * jcp.iw;
...
#pragma omp parallel for  num_threads(num_thr)
    ...
    // FIXME identical with jit_gemm_convolution_utils
void init_conf(
    any_gemm_conv_conf_t &jcp, const convolution_desc_t &cd,
...
status_t prepare_workspace(
        any_gemm_conv_conf_t &jcp, float **ws, bool is_bwd_weights,
...
void bwd_weights_balance(int ithr, int nthr, int ngroups, int mb, int &ithr_g,
    ...
    // FIXME identical with jit_gemm_convolution_utils
void bwd_weights_reduction_par(int ithr, int nthr, const any_gemm_conv_conf_t &jcp,
