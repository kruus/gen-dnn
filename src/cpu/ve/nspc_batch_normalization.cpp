...
#include "cpu/ve/nspc_batch_normalization.hpp"
...
template<typename T>
struct MaybePostOp { // an "optional relu" op
...
void nspc_batch_normalization_fwd_t<d_type>::execute_forward(
    ...
    // VE doesn't inline and vectorize lambdas very well yet
...
#if 0 // x86 (conditions inside loop OK)
#if SAFE_TO_USE_OMP_SIMD
                    PRAGMA_OMP_SIMD()
#endif
                    for (int c = 0; c < C; c++) {
...
#if defined(__ve)
    // TODO remove tail loop for ve, remove c blocking ??
    constexpr dim_t c_blk = cpu_isa_traits<ve_common>::mvl; // 256
#else
    ...
    parallel(nthr, [&](const int ithr, const int nthr) {
        ...
        for (dim_t n = N_s; n < N_e; n++) {
            for (dim_t sp = 0; sp < SP; sp++) {
                ...
                if (fuse_norm_relu) {
                ...
                }else{ // !fuse_norm_relu can avoid non-vectorizing uint8 ws[]
                    for (dim_t c = 0; c < nb_c_blk * c_blk; c++) {
                        const size_t c_off = s_off + c;
                        acc_data_t gamma = use_scaleshift ? scaleshift[c] : 1;
                        acc_data_t sqrt_variance = static_cast<acc_data_t>(
                                1.0f / sqrtf(variance[c] + eps));
                        acc_data_t v_diff_src = _diff_dst[c];
                        if (calculate_diff_stats) {
                            v_diff_src -= diff_beta_loc[c] / (SP * N)
                                    + (_src[c] - mean[c]) * diff_gamma_loc[c]
                                            * sqrt_variance / (SP * N);
                    ...
                    for (dim_t c = 0; c < tail; c++) {
                        ...
                        acc_data_t sqrt_variance = static_cast<acc_data_t>(
                                1.0f / sqrtf(variance[nb_c_blk * c_blk + c] + eps));
                        acc_data_t v_diff_src = _diff_dst[nb_c_blk * c_blk + c];
                        if (calculate_diff_stats) {
                            v_diff_src -= diff_beta_loc[nb_c_blk * c_blk + c]
                                            / (SP * N)
                                    + (_src[nb_c_blk * c_blk + c]
