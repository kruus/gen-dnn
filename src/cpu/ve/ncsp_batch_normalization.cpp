...
template<typename T>
struct MaybePostOp {
    MaybePostOp(bool const with_relu) : with_relu(with_relu) {}
    T constexpr operator()(T const res) const {
        return (with_relu && res < 0) ? 0 : res;
    }
    bool const with_relu;
};
...
void ncsp_batch_normalization_fwd_t<d_type>::execute_forward(
    ...
    MaybePostOp<acc_data_t> const maybe_post_op(with_relu);
    const bool has_spatial = utils::one_of(pd()->ndims(), 4, 5);
    const dim_t SP = (has_spatial) ? pd()->H() * pd()->W() * pd()->D() : 1;
#if defined(__ve)
    constexpr dim_t simd_w = cpu_isa_traits<ve_common>::vlen; // MVL=256
    ...
    parallel(0, [&](const int ithr, const int nthr) {
        ...
        for (int64_t it = 0; it < iters; ++it) {
            ...
            for (dim_t c = C_blk_s; c < C_blk_e; c++) {
                ...
                if (d_type == bf16) {
                    for (dim_t n = N_s; n < N_e; ++n) {
                        acc_data_t * __restrict__ _dst;
                        const acc_data_t * __restrict__ _src;
                        ...
                        // ve: uint8_t ws (byte stores) do not vectorize
#if 0
...
#else
                        if (fuse_norm_relu) {
                            if (is_training) { // byte choice for ws not vectorizable :(
                                IVDEP() ShortLoopTest()//;
                                for (dim_t sp = S_s; sp < S_e; ++sp) {
                                    size_t d_off = s_off + sp;
                                    acc_data_t bn_res = sm * (_src[sp] - mean[off]) + sv;
                                    if (bn_res <= 0) {
                                        bn_res = 0;
                                        ws[d_off] = 0; // VE: uint8_t is an unvectorizable data type
                                    } else {
                                        ws[d_off] = 1;
                                    }
                                    _dst[sp] = maybe_post_op(bn_res);
                                ...
                                for (dim_t sp = S_s; sp < S_e; ++sp) {
                                    size_t d_off = s_off + sp;
                                    acc_data_t bn_res = sm * (_src[sp] - mean[off]) + sv;
                                    if (bn_res <= 0) bn_res = 0;
                                    _dst[sp] = maybe_post_op(bn_res); // no effect?
                                }
                            }
                        }else{
                            ShortLoopTest()//;
...
void ncsp_batch_normalization_bwd_t<d_type>::execute_backward(
    ...
    const dim_t SP = (has_spatial) ? pd()->H() * pd()->W() * pd()->D() : 1;
#if defined(__ve)
    constexpr dim_t simd_w = cpu_isa_traits<ve_common>::mvl; // MVL=256
#else
    const dim_t simd_w = 16;
#endif
    const dim_t SP_cl_align = utils::rnd_up(SP, simd_w);
    ...
    parallel(0, [&](const int ithr, const int nthr) {
        ...
        for (int64_t it = 0; it < iters; ++it) {
            ...
            for (dim_t c = C_blk_s; c < C_blk_e; c++) {
                ...
                if (d_type == bf16) {
                    for (dim_t n = N_s; n < N_e; ++n) {
                        const acc_data_t * __restrict__ _diff_dst;
                        const acc_data_t * __restrict__ _src;
            ...
            for (dim_t c = C_blk_s; c < C_blk_e; c++) {
                ...
                acc_data_t v_mean = mean[off];
                if (d_type == bf16) { // VE: don't care so much
                    for (dim_t n = N_s; n < N_e; ++n) {
                        ...
                        if (calculate_diff_stats) {
                            ...
                            _src = nullptr; // avoid gcc483 compiler warning
                        }
#if SAFE_TO_USE_OMP_SIMD
                        PRAGMA_OMP_SIMD()
#endif
