...
// ncc issue with multiple OMP clauses in same function :( at least for older ncc)
void gemm_convolution_bwd_weights_t::execute_backward_weights(
...
#if VE_OPENMP_BUG
        execute_backward_weights_reduction(ctx);
#else
        parallel(jcp.nthr, [&](const int ithr, const int nthr) {
            int ithr_g, nthr_g, ithr_mb, nthr_mb;
    ...
    if (jcp.with_bias) {
#if VE_OPENMP_BUG
        execute_backward_weights_bias(ctx);
#else
...
void gemm_convolution_bwd_weights_t::execute_backward_weights_bias(
    ...
    const dim_t K = jcp.os * jcp.od;
    const size_t dst_step = jcp.oc * K;
    parallel_nd(jcp.ngroups, jcp.oc, [&](int_t const g, int_t const oc) {
        data_t db = 0;
        size_t offset_ = (size_t)g * dst_step + (size_t)oc * K;
        FOR(mb,jcp.mb) {
            size_t offset = offset_ + (size_t)mb * jcp.ngroups * dst_step;
            FOR(od,jcp.od)
            FOR(oh,jcp.oh)
            PRAGMA_OMP_SIMD(reduction(+ : db)) PragmaQuote(_NEC shortloop_reduction)//;
            FOR(ow,jcp.ow) {
                db += diff_dst[offset];
                ++offset;
            }
...
void gemm_convolution_bwd_weights_t::execute_backward_weights_reduction(
        const exec_ctx_t &ctx) const {
    auto diff_weights = CTX_OUT_MEM(data_t *, DNNL_ARG_DIFF_WEIGHTS);
    auto wei_reduction
            = ctx.get_scratchpad_grantor().get<data_t>(key_conv_wei_reduction);
