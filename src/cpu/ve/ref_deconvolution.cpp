...
void ref_deconvolution_fwd_t::compute_fwd_bias(
...
#if defined(__ve)
    const memory_desc_wrapper_opt dst_d(pd()->dst_md());
#else
...
#else // 1st option: split over nelems
    // UNTESTED XXX new usage in ve/memory_desc_wrapper_opt.hpp
    const ptrdiff_t nelems = static_cast<ptrdiff_t>(dst_d.nelems(false));
    parallel(0, [&](const int ithr, const int nthr) {
            ...
            auto cf= (ndims >= 5)? Coords::mk(0,MB, 0,pd()->OC(), 0,OD, 0,OH, 0,OW)
                : (ndims >= 4)? Coords::mk(0,MB, 0,pd()->OC(), 0,OH, 0,OW)
                : Coords::mk(0,MB, 0,pd()->OC(), 0,OW);
            // restrict coord generation to subrange,
            // we CAN modify cf.vp coords inside, since
            // ++cf will recalc all cords based on linear pos
            NOVEC_ for(cf.init_nd(start,end); cf; ++cf)
            //for(cf.init_nd(start,end); cf.get_vl()>0; ++cf)
            //cf.init_nd(start,end);
            //for( ; cf.vl>0; ++cf)
            {
                auto const vl = cf.get_vl();
                dim_t chan[MVL]; // copy channel (potentially destroyed by _vtmp fn)
                ShortLoop() for(int i=0; i<vl; ++i)
                    chan[i] = cf.vp[1][i];
                dim_t poff[MVL];      // array of physical offsets
                bool const is_pos_padded = true; // What is correct here XXX ?
                // NEW "cf.vp is non-const" function public ...
                dst_d.vec_off_vtmp(cf, poff, vl, is_pos_padded);
                IVDEP() ShortLoop() for(int i=0; i<vl; ++i){
                    auto b = bias[chan[i]]; // old ~ g*OC + oc
                    dst[poff[i]] += b;
                }
            }
    });
...
void ref_deconvolution_fwd_t::compute_fwd_bias_ncdhw(
    ...
    // XXX Feedback of scalar value from one loop pass to another.: d1, d0
    //      (from for_nd in dnnl_thread_parallel_nd.hpp)
    parallel_nd(MB, OC, [&](int mb, int oc) {
        auto const bias_oc = bias[oc];
        auto const off0 = (size_t)(mb * OC + oc) * SP;
...
#if 0 // crude (should use CoordsForNd and vec_off_vtmp ?)
    switch (ndims) {
        case 5:
...
void ref_deconvolution_bwd_weights_t::compute_bwd_bias_ncdhw(
    ...
    parallel_nd(OC, [&](int oc) {
        float db = 0;
        PragmaQuote(_NEC shortloop_reduction) //; XXX
        for (int mb = 0; mb < MB; ++mb) {
...
void ref_deconvolution_bwd_weights_t::compute_bwd_bias_nCdhwXc(
    ...
    // XXX 8,16 too short VL for Aurora (reblock sp loops & vec gather?)
