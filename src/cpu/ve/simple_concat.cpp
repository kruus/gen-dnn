...
status_t simple_concat_t<data_type>::execute(const exec_ctx_t &ctx) const {
    ...
    if (!has_outer_loop) {
        // XXX VE: alt parallelization of this case? (particularly for < 4-byte data_type)
        for (int a = 0; a < num_arrs; ++a) {
            const data_t *i = &iptrs[a][0];
            data_t *o = &optrs[a][0];
            // XXX handle this with chunking into vectorizable and nonvectorizable
            //     (memcpy if possible, as in utils::array_copy) branches
            //if (sizeof(data_type) == 4U) {
    ...
    parallel_nd(phys_dims[0], phys_dims[1], phys_dims[2], phys_dims[3],
            ...
            [&](dim_t n0, dim_t n1, dim_t n2, dim_t n3, dim_t n4, int a) {
                ...
                if (sizeof(data_type==4)) {
                    ShortLoopTest() for (dim_t e = 0; e < nelems_to_copy[a]; ++e)
                        o[e] = i[e];
                }else{
