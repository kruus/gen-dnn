...
status_t conv_desc_init(convolution_desc_t *conv_desc, prop_kind_t prop_kind,
    ...
    Consistency args_ok("conv_args_check");
#define AND_(...) SCHKVV(args_ok,__VA_ARGS__)
    AND_(!any_null(conv_desc, src_desc, weights_desc, dst_desc, strides,
                   padding_l));
    AND_(one_of(alg_kind, convolution_auto, convolution_direct,
...
#define AND_(...) SCHKVV(ok,__VA_ARGS__)
    ...
    AND_(utils::one_of(weights_desc->ndims, src_desc->ndims, src_desc->ndims + 1));
